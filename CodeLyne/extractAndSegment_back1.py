...
##                imgname = '../Caltech1999/image_0114.jpg'
##                imgBGR = cv2.imread(imgname,1)
##                name = 'image_0114'
                #Display image
...
##                        if len(horStarts) != 0:
##                                plotBoundingRegionsCV2(imgBGRCopy, horStarts+x, horEnds+x, base+y, height)
##                                readPlate(gray2, horStarts+x, horEnds+x, base+y, height)
##                dirLocation = 'SegmentationOutput/'
##                outfile = dirLocation+ name + '_seg.png'
##                print outfile
##                cv2.imwrite(outfile, imgBGRCopy)
##                cv2.imshow('IMG2',imgBGRCopy)
##                cv2.waitKey(0)
        ...
        for i in xrange(len(starts)):
                ...
                for j in range(len(letterNum)):
                        ...
                        for scale in np.linspace(0.5, 1, 10):
                                ...
                                #cv2.imshow('Resized',tempResized)
                                #cv2.waitKey(0)
                                #cv2.destroyAllWindows()
                                ...
                                alpha = 0
                                #print "firstVal", max_val
                                maxval = max_val -alpha*(sum(sum(x) for x in tempResized)- sum(sum(x) for x in character))
                                #print "secVal", (sum(sum(x) for x in tempResized)- sum(sum(x) for x in character))
                                #print "maxval" , str(max_val)
                                #print "maxRunning" , maxRunning
                                #print "Score for " + char + " =", max_val
                                if i in [1, 2 ,3]:
                                        if j <= 9:
                                                max_val = -float("inf")
                                                #print "ij1", i, " ", j
...
##                                ret1,thresh1 = cv2.threshold(character,127,255,0)
##                                ret2,thresh2 = cv2.threshold(tempResized,127,255,0)
##                                contours1,hierarchy1 = cv2.findContours(thresh1,2,1)
##                                contours2,hierarchy2 = cv2.findContours(thresh2,2,1)
##                                cnt1 = contours1[0]
##                                cnt2 = contours2[0]
##                                min_val = cv2.matchShapes(cnt1,cnt2,1,0.0)
##                                minRunning = min(minRunning,min_val)
                        scores.append(maxRunning)
                        #scores.append(minRunning)
                #print "scores" ,[(letterNum[i], scores[i+1]) for i in range(len(letterNum))]
                mx = np.argmax(np.array(scores[1:]))
                #mn = np.argmin(np.array(scores[1:]))
